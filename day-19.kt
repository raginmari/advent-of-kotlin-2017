...
fun solve(puzzleInput: List<String>): String {

  var path = parsePuzzleInput(puzzleInput)
  val orderedLetters = walkPath(path)

  val result = orderedLetters.joinToString("")

  return result
}

fun walkPath(path: Map<Pair<Int, Int>, PathItem>): List<Char> {
  ...
  while (true) {
    ...
    val letter = path.get(Pair(x, y))!!.letter
    if (letter != null) {
      println("Reading ${letter}")
      result.add(letter)
    }
  }
}
